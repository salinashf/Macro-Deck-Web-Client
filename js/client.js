...
$(document).ready(function () {
  ...
  if (getCookie("recentConnections")) {
    ...
    for (var i = 0; i < recentConnections.length; i++) {
      ...
      document
        .getElementById("recent-connections")
        .appendChild(recentConnectionItemRow);
    }
  }
});
...
function connect(url) {
  ...
  websocket.onopen = function (e) {
    ...
    document.getElementById("connect-container").innerHTML =
      '<div class="d-flex align-items-center justify-content-center" style="height: 500px;"><h1>Waiting for accepting the connection... <span class="spinner-border spinner-border-lg" role="status" aria-hidden="true"></span></h1></div>';
    var jsonObj = {
      Method: JsonMethod.CONNECTED,
      "Client-Id": clientId,
      API: apiVersion,
      "Device-Type": "Web",
    };
      ...
      switch (obj.Method) {
        case JsonMethod.GET_CONFIG:
          ...
          if (
            obj.Columns !== columns ||
            obj.Rows !== rows ||
            obj.ButtonBackground != buttonBackground
          ) {
          ...
          buttonBackground = obj.ButtonBackground;
          if (
            obj.SupportButtonReleaseLongPress &&
            obj.SupportButtonReleaseLongPress == true
          ) {
        ...
        case JsonMethod.GET_BUTTONS:
          ...
          for (var i = 0; i < this.buttons.length; i++) {
            var button = document.getElementById(
              this.buttons[i].Position_Y + "_" + this.buttons[i].Position_X
            );
            ...
            if (button && this.buttons[i]) {
              if (this.buttons[i].Icon) {
                ...
                if (Array.prototype.find != null) {
                  // using faster find method for supported browser
                  ...
                  // using slower for loop to find the icon for older browsers
                ...
                button.style.backgroundImage =
                  "url(data:image/gif;base64," + icon.IconBase64 + ")";
              } else if (this.buttons[i].IconBase64) {
                button.style.backgroundImage =
                  "url(data:image/gif;base64," +
                  this.buttons[i].IconBase64 +
                  ")";
              }
            }
            ...
            if (label) {
              if (this.buttons[i].Label && this.buttons[i].Label.LabelBase64) {
                label.style.backgroundImage =
                  "url(data:image/gif;base64," +
                  this.buttons[i].Label.LabelBase64 +
                  ")";
              }
              if (this.buttons[i].LabelBase64) {
                label.style.backgroundImage =
                  "url(data:image/gif;base64," +
                  this.buttons[i].LabelBase64 +
                  ")";
              }
            }
            ...
            if (button && this.buttons[i].BackgroundColorHex) {
              button.classList.toggle(
                "btn-secondary",
                !this.buttons[i].BackgroundColorHex
              );
        ...
        case JsonMethod.UPDATE_BUTTON:
          var button = document.getElementById(
            obj.Buttons[0].Position_Y + "_" + obj.Buttons[0].Position_X
          );
          ...
          if (button) {
            if (obj.Buttons[0].Icon) {
              ...
              if (Array.prototype.find != null) {
                // using faster find method to find the icon for supported browsers
                iconPack = icons.IconPacks.find(function (e) {
                ...
                for (var j = 0; j < icons.IconPacks.length; j++) {
                  if (
                    ...
                    icons.IconPacks[j].Name == obj.Buttons[0].Icon.split(".")[0]
                  ) {
                    iconPack = icons.IconPacks[j];
                  }
                }
              ...
              button.style.backgroundImage =
                "url(data:image/gif;base64," + obj.Buttons[0].IconBase64 + ")";
            } else {
              button.style.backgroundImage = "";
            }
            ...
            if (button && obj.Buttons[0].BackgroundColorHex) {
              button.classList.toggle(
                "btn-secondary",
                !obj.Buttons[0].BackgroundColorHex
              );
          ...
          if (label) {
            if (obj.Buttons[0].Label && obj.Buttons[0].Label.LabelBase64) {
              label.style.backgroundImage =
                "url(data:image/gif;base64," +
                obj.Buttons[0].Label.LabelBase64 +
                ")";
            }
            if (obj.Buttons[0].LabelBase64) {
              label.style.backgroundImage =
                "url(data:image/gif;base64," + obj.Buttons[0].LabelBase64 + ")";
            }
          }
          break;
...
function autoSize() {
  ...
  for (i = 0; i < buttons.length; i++) {
    buttons[i].style.borderRadius =
      (buttonRadius / 2 / 100) * buttonSize + "px";
  }
}
...
function buttonPress(id) {
  ...
  if (document.getElementById("col_" + id)) {
    ...
    document
      .getElementById("col_" + id)
      .classList.toggle("release-transition", false);
  }
...
function buttonPressRelease(id) {
  ...
  if (document.getElementById("col_" + id)) {
    ...
    document
      .getElementById("col_" + id)
      .classList.toggle("release-transition", true);
  }
